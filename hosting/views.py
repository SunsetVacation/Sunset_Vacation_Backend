...
from .models import Category, Hosting, Property, Facility,Property_Images,Property_Facilities,Location
...
@api_view(['POST'])
def getHostingList(request):
    try:
        print("id =  ", request.data["id"])
        user = User.objects.get(user_id=request.data["id"])
        print("user = ", user)
        hostingList = Hosting.objects.filter(owner=user)
        print("hosting list = ", hostingList)
    except:
        return Response({"error": "No hosting to show"}, status=status.HTTP_404_NOT_FOUND)
    hosting_serializer = HostingSerializer(hostingList, many=True)

    images = []
    for i in hosting_serializer.data:
        image =  Property_Images.objects.filter(hosting=i['hosting_id'])
        print(image)
        image_serializer = PropertyImagesSerializer(image, many=True)
        images.append(image_serializer.data)
    print(images)

    return Response({'hostingList':hosting_serializer.data, 'images':images,  'success': True}, status=status.HTTP_200_OK)
...
class PropertyHostingView(
    ...
    #     user = User.objects.get(user_id=request.data['owner_id'])
    #     user.host = True
    #     user.save()
    ...
    def post(self, request):
        ...
        return Response({"hosting": hosting_serializer.data, "property": property_serializer.data, "location": location_serializer.data, "facilities": facilities_serializer.data, "images": images_serializer.data, "success": True},
                        status=status.HTTP_201_CREATED)
    ...
    def get(self, request, hosting_id=None, *args, **kwargs):
        if hosting_id:
            ...
            # print(facilities)
            facilities_serializer = PropertyFacilitiesSerializer(facilities, many=True)
            # print(facilities_serializer)


            facilityIdList = []
            for i in facilities_serializer.data:
                # facility_name = Facility.objects.filter(facility=i['facility'])
                print(i['facility'])
                facilityIdList.append(i['facility'])
            # facility_name_serializer = Facility(facility_name, many=True)
            # print(facility_name_serializer)

            facilityNames = []
            for i in facilityIdList:
                x = Facility.objects.get(facility_id=i)
                x_serialize = FacilitySerializer(x)
                # print(x_serialize.data)
                facilityNames.append(x_serialize.data['facility_name'])
            
            print(facilityNames)

            return Response({"hosting": hosting_serializer.data, "property": property_serializer.data, "location": location_serializer.data, "facility_names": facilityNames,"facilities": facilities_serializer.data, "images": images_serializer.data},
                            status=status.HTTP_200_OK)
        else:
            try:
                property = Property.objects.all()
